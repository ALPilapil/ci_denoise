...
class UNet(nn.Module):
    def __init__(self):
        ...
        self.down_conv_1 = double_conv(21, 64)
        ...
        self.out = nn.Conv1d(
            in_channels=64,
            out_channels=21, # can adjust this based on num objects to segment 
            kernel_size=1,
        )
    ...
    def forward(self, image):
        ...
        x = self.up_conv_1(torch.cat([x7[..., :x.shape[-1]], x], 1))

        x = self.up_trans_2(x)
        x = self.up_conv_2(torch.cat([x5[..., :x.shape[-1]], x], 1))

        x = self.up_trans_3(x)
        x = self.up_conv_3(torch.cat([x3[..., :x.shape[-1]], x], 1))

        x = self.up_trans_4(x)
        x = self.up_conv_4(torch.cat([x1[..., :x.shape[-1]], x], 1))
